(() => {
  ...
  const CONFIG = {
    ...
    refreshMinutes: 60,
    cacheKey: "torre-petrorec-cache-v1",
  ...
  function saveCache(rows) {
    try {
      localStorage.setItem(CONFIG.cacheKey, JSON.stringify({ updatedAt: new Date().toISOString(), rows }));
    } catch (_) { /* navegador pode bloquear armazenamento */ }
  }

  function readCache() {
    try {
      const cached = JSON.parse(localStorage.getItem(CONFIG.cacheKey) || "null");
      if (Array.isArray(cached?.rows) && cached.rows.length) return cached;
    } catch (_) { /* cache ausente ou inválido */ }
    return null;
  }
  ...
  async function refreshData({ silent = false } = {}) {
    ...
    try {
      ...
      render();
      setStatus(
        "Planilha PetroRec sincronizada",
        "ok",
      ...
      if (cached) {
        allRows = cached.rows;
        lastUpdatedAt = new Date(cached.updatedAt);
        populateFilters();
        render();
        setStatus(
          "Sem conexão · última carga preservada",
          "cache",
          `${allRows.length} registros · cache de ${lastUpdatedAt.toLocaleString("pt-BR")}`
        );
      } else {
        setStatus("Não foi possível carregar a base", "error", error.message);
        showEmptyState();
      }
  ...
  function init() {
    ...
    if (cached) {
      ...
      populateFilters();
      render();
      setStatus("Última carga preservada", "cache", `${allRows.length} registros · cache de ${lastUpdatedAt.toLocaleString("pt-BR")}`);
    }
